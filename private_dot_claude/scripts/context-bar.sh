...
C_COST_LOW='\033[38;5;136m'   # gold  — normal spend
C_COST_MED='\033[38;5;214m'   # orange — getting pricey (>= $1)
C_COST_HIGH='\033[38;5;196m'  # bright red — expensive (>= $5)
case "$COLOR" in
...
if [[ -n "$cost_usd" ]]; then
    ...
    cost_segment=" | ${cost_color}\$${cost_display}${C_GRAY}"
fi

# Build output: Model | Dir | Branch (uncommitted) | Context | Cost
output="${C_ACCENT}${model}${C_GRAY} | 📁${dir}"
[[ -n "$branch" ]] && output+=" | 🔀${branch} ${git_status}"
output+=" | ${ctx}${cost_segment}${C_RESET}"
...
if [[ -n "$transcript_path" && -f "$transcript_path" ]]; then
    ...
    [[ -n "$cost_usd" ]] && plain_output+=" | \$$(printf '%.4f' "$cost_usd")"
